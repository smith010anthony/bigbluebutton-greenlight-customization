...
class SessionsController < ApplicationController
  ...
  # we can use it for admin login
  # POST /users/login_old
  def create_old
  ...
  def create
    ...
    logger.info "[session] @create user : #{user.inspect}."
    ...
    unless is_super_admin
      logger.info "[session] @create user 2 : #{user.inspect}."

      uri = URI('https://w81la6a127.execute-api.us-east-2.amazonaws.com/login')
      http = Net::HTTP.new(uri.host, uri.port)
      http.use_ssl = true

      request = Net::HTTP::Post.new(uri.path, {'Content-Type' => 'application/json'})

      logger.info "[session] @create request : #{request.inspect}."

      request.body = {
        "route": "login",
        "payload": {
                "type": "manual",
                "email": session_params[:email].downcase,#"conner@mail.com",#
                  "password": session_params[:password] #"conner",#
            }
      }.to_json
      #"angel@mail.com",
      #"angel"
      ...
      logger.info "[session] @create api response body : #{body}. status: #{body['status']}"

      if "ok" == body["status"]
        #user is available if this user doesn't exist in database than create one
        unless user
          logger.info "[session] @create new user name : #{body["data"]["profile"]["name"]}"
          #ttt="aaa"
          # also add jwt_token => body.data.token
          #user_params={"name"=>body["data"]["profile"]["name"], "email"=>session_params[:email].downcase, "password"=>session_params[:password], "password_confirmation"=>session_params[:password],"jwt_token" => body["data"]["token"]}.to_json 
          
          #@user = User.new(name:body["data"]["profile"]["name"],email:session_params[:email].downcase,password:session_params[:password],password_confirmation:session_params[:password],jwt_token:body["data"]["token"])
          fullname=body["data"]["profile"]["name"]
          @user = User.new(name:fullname,email:session_params[:email].downcase,password:session_params[:password],password_confirmation:session_params[:password],jwt_token:body["data"]["token"])
          
          #@user = User.new(name:'test',email:session_params[:email].downcase,password:session_params[:password],password_confirmation:session_params[:password],jwt_token:'tt')
          @user.provider = @user_domain
          @user.save
          # no email varification required so set user role
          logger.info "[session] @create new user : #{@user.inspect}"

          #true this if for all case
          #if approval_registration || !Rails.configuration.enable_email_verification
            if(body["data"]["profile"]["role"]=='professor')
              @user.set_role(initial_user_role(@user.email))
              @user.update(room_id: ENV["ROOM_NUM_ID"])
              login(@user)
            else
              # if user is student
              @user.set_role_without_home_room(initial_user_role(@user.email))
              session[:user_id] = @user.id
              @user.update(last_login: Time.zone.now)
              # @user.update(room_id: ENV["ROOM_NUM_ID"])
              logger.info(" root_path: #{root_path}")
              redirect_to root_path+"/"+body["data"]["profile"]["roomId"]
            end
          #end

          # logger.info "[session] @create new user 1 : #{@user.inspect}"

          # session[:user_id] = user.id
          # user.update(last_login: Time.zone.now)

          # logger.info("Support: #{user.email} has successfully logged in. redirect ",root_path+"/"+body["data"]["profile"]["room_id"])
          # redirect_to root_path+"/"+body["data"]["profile"]["room_id"]
          #login(@user)
        else
          user.update_attribute(:jwt_token, body["data"]["token"])
          user.update(last_login: Time.zone.now)
          session[:user_id] = user.id
          logger.info(" root_path: #{root_path}")
          redirect_to root_path+"/"+body["data"]["profile"]["roomId"]
      ...
      # just to allow simple login for super user
      return redirect_to(signin_path, alert: I18n.t("invalid_credentials")) unless user
