...
module Authenticator
  ...
  def login(user)
    ...
    if !Rails.configuration.terms || user.accepted_terms
      logger.info("Support: #{user.email} if .")
      check_email_verified(user)
    else
      logger.info("Support: #{user.email} else .")
  ...
  def check_email_verified(user)
    ...
    if user.has_role? :super_admin
      ...
      logger.info("Support: @check_email_verified permission #{user.role.get_permission("can_create_rooms")} ")
      ...
      logger.info("Support: @check_email_verified  account_activation_path #{account_activation_path(digest: user.activation_digest)}")
