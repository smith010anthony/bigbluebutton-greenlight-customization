...
class RoomsController < ApplicationController
  ...
  def join
    ...
    logger.info "Support: #{current_user.present? ? current_user.email : @join_name} is joining room #{@room.uid} #{default_meeting_options.inspect}"
  ...
  def join_specific_room
    ...
    logger.info "Support: join_specific_room #{room_uid}"
  ...
  def start
    ...
    uri = URI('https://vapxx8gela.execute-api.eu-west-1.amazonaws.com/connect')
    http = Net::HTTP.new(uri.host, uri.port)
    http.use_ssl = true

    request = Net::HTTP::Post.new(uri.path, {'Content-Type' => 'application/json'})

    logger.info "Support: [room] @create request : #{request.inspect}."
    ...
    logger.info "Support: [room] @connect api response body : #{body}. status: #{body['status']}"

    if "ok" == body["status"]
      logger.info "Support: [room] @connect api response endpoint #{body['data']['endpoint']}"
      opts["userdata-bbb_real_engine_address"]=body['data']['endpoint']
    end
    logger.info "Support @start room #{@room.inspect} ,opts #{opts.inspect}"
    joinpath=join_path(@room, current_user.name, opts, current_user.uid)
    begin
      logger.info "Support: @start #{current_user.email} is starting room join_path #{joinpath}"
  ...
  def verify_room_owner_valid
    logger.info "[rooms] @verify_room_owner_valid room : #{@room.inspect}"
    # logger.info "[rooms] @verify_room_owner_valid @room.owner : #{@room.owner.inspect}"
