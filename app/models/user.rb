...
class User < ApplicationRecord
  ...
  def set_role(role) # rubocop:disable Naming/AccessorMethodName
    ...
    logger.info "[user] @set_role self: #{self.inspect}."
    
    new_role = Role.find_by(name: role, provider: role_provider)

    return if new_role.nil?

    logger.info "[user] @set_role role: #{role}."
  ...
  def set_role_without_home_room(role) # rubocop:disable Naming/AccessorMethodName
    return if has_role?(role)
  ...
  def create_home_room
    logger.info "[user] @create_home_room self: #{self.inspect}."
    room = Room.create!(owner: self, name: I18n.t("home_room"))
    logger.info "[user] @create_home_room room: #{room.inspect}."
  ...
  def setup_user
    ...
    logger.info "[user] @setup_user id: #{id}."
